...
usage()
{
echo "# This is a script designed to combine .csv output from the ImageJ yH2AX pipeline.
...
#
# Usage: bash nuclear_area_combine_csv.sh -p output_prefix -v vector_name -l coverslip_information -s pattern_to_identify_csvs_to_cobine
#
# This script takes as input 3 REQUIRED options and 1 OPTIONAL options.
# Options
# -p    REQUIRED.       Prefix used for combined output file.
# -l    REQUIRED.       Tezt to go into the second column of the .csv output to identify coverslip cell were imaged from.
# -v	REQUIRED.		Vector cells were transfected with.
# -s    REQUIRED.       Pattern to identify .csv files. This pattern goes after a * to identify specific .csv files you want to analyse within the current directory. (Default: .csv)
...
if [ -z "${pattern}" ]; then # If no pattern provided, .csv will be set as default
	usage
	echo "## ERROR: You need to provide a pattern to identify csvs with"
	exit 1
fi
...
for i in $(ls ${pattern});
do
	awk -v vector=${vector} -v line=${COUNTER} -v title=${label} 'NR > 1 {print vector","title"_"line"_"$0}' ${i} >> ${prefix}_nuclear_area_combined.txt
	COUNTER=$((COUNTER+1))
done
