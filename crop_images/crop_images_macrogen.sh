...
usage() {
echo "# Script to generate macro that crops and save images to pre-determined size
...
echo "// Crop Image Complete Macro" > crop_image_complete_IJ_macro.ijm
...
cat ${1} | while read r; do
	...
	-e "s@DIM3@${DIM3}@g" \
	-e "s@DIM4@${DIM4}@g" \
	>> crop_image_complete_IJ_macro.ijm;
done